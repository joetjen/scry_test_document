defmodule Scry.Test.Document.MixProject do
  use Mix.Project
  ...
  defp deps do
    [
      # === SCRY CORE / SCRY DOCUMENT ===
      # Local path dependencies, not Hex version constraints, since
      # neither is published to Hex yet. Both real, not test-only:
      # this package parses query text through Scry.Document.parse/1
      # and drives Scry.Document.Executor.run/3 directly.
  ...
  defp description do
    "Shared test fixtures for scry_document: one seed dataset servable through " <>
      "Scry.Test.Document.Conn's own tree/0 constructor, plus scry_core's own mix " <>
      "scry.query/scry.iex (configured here) for exercising it. Not a cross-backend parity " <>
      "suite -- scry_document has exactly one executor, no interchangeable EngineBehaviour " <>
      "backends to compare."
  end
  ...
  defp package do
    [
      licenses: ["MIT"],
      links: %{"GitHub" => "https://github.com/joetjen/scry_test_document"},
  ...
  defp docs do
    [
      main: "readme",
      source_url: "https://github.com/joetjen/scry_test_document",
